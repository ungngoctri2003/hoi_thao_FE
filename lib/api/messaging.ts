...
export const messagingApi = {
  ...
  async getConferenceAttendees(conferenceId: number): Promise<ChatContact[]> {
    try {
      ...
      try {
        ...
        console.log('Fallback: Using all users as contacts:', allUsersResponse.data);
        ...
        return allUsersResponse.data.map((user: any) => ({
          ...
          phone: '',
          company: '',
          position: user.role === 'admin' ? 'Quản trị viên' : 
                   user.role === 'staff' ? 'Nhân viên' : 
                   user.userType === 'attendee' ? 'Người tham dự' : 'Người dùng',
          avatar: user.avatar,
  ...
  async getSessionMessages(sessionId: number, page: number = 1, limit: number = 50): Promise<{ messages: Message[], total: number }> {
    try {
      // Use direct fetch since we don't have a public method for this
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/sessions/${sessionId}/messages?page=${page}&limit=${limit}`, {
        method: 'GET',
        headers: {
  ...
  // Gửi tin nhắn
  async sendMessage(sessionId: number, content: string, type: 'text' | 'image' | 'file' = 'text', attendeeId?: number): Promise<Message> {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/sessions/${sessionId}/messages`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken') || ''}`
  ...
  async searchAttendees(conferenceId: number, query: string): Promise<ChatContact[]> {
    try {
      const response = await api.getAttendees({ 
        page: 1, 
        limit: 50,
  ...
  async searchAllUsers(query: string): Promise<ChatContact[]> {
    try {
      ...
      console.log('User types and roles:', response.data.map((u: any) => ({ 
        name: u.name, 
        email: u.email, 
        role: u.role, 
        userType: u.userType 
      ...
      return filteredUsers.map((user: any) => ({
        ...
        phone: '',
        company: '',
        position: user.role === 'admin' ? 'Quản trị viên' : user.role === 'staff' ? 'Nhân viên' : 'Người dùng',
        avatar: user.avatar,
        isOnline: Math.random() > 0.5,
        lastSeen: 'Không xác định',
        lastMessage: undefined,
  ...
  async getOrCreateConversationSession(conferenceId: number, attendeeId: number): Promise<number> {
    try {
      // First, try to get existing sessions for this conference
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/conferences/${conferenceId}/sessions`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken') || ''}`
      ...
      const createResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/conferences/${conferenceId}/sessions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken') || ''}`
