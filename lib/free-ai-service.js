// Free AI service using Hugging Face Inference API (JavaScript version)
class FreeAIService {
  constructor() {
  ...
  generateFallbackInsights(analyticsData) {
    const insights = [];
  ...
  async generateAnalyticsInsights(analyticsData) {
    try {
      ...
      return {
        insights,
        summary: "Phân tích được thực hiện bởi AI (Hugging Face).",
        recommendations: [
  ...
  async generateConferenceRecommendations(conferenceId, conferenceName, data) {
    try {
...
module.exports = { freeAIService: new FreeAIService() };
