...
export class AuthService {
  ...
  private async initializeAuth(): Promise<void> {
    try {
      ...
      if (token) {
        ...
        console.log('Auth initialized with valid token, user:', this.state.user);
        ...
        notificationService.success(
          ...
          if (errorMessage.includes('Phiên đăng nhập đã hết hạn') || errorMessage.includes('401')) {
          ...
          } else if (errorMessage.includes('Tài khoản đã bị khóa') || errorMessage.includes('Account is disabled')) {
    ...
    try {
      ...
      try {
        ...
        this.state.user = {
          ...
          role: 'attendee',
          permissions: ['dashboard.view', 'profile.view', 'networking.view', 'venue.view', 'sessions.view', 'badges.view', 'mobile.view', 'my-events.view'],
        };
        this.state.isAuthenticated = true;
      ...
      if (errorMessage.includes('Tài khoản đã bị khóa') || errorMessage.includes('Account is disabled')) {
        toast({
      ...
      } else if (errorMessage.includes('401') || errorMessage.includes('Unauthorized') || errorMessage.includes('Invalid credentials')) {
        toast({
          title: "Đăng nhập thất bại",
          description: "Email hoặc mật khẩu không chính xác. Vui lòng kiểm tra lại.",
          variant: "destructive",
          duration: 6000,
        });
      } else if (errorMessage.includes('404') || errorMessage.includes('User not found')) {
        toast({
          title: "Tài khoản chưa được đăng ký",
          description: "Email này chưa được đăng ký. Vui lòng đăng ký tài khoản trước.",
          variant: "destructive",
          duration: 6000,
        });
  ...
  public async register(email: string, name: string, password: string): Promise<void> {
    try {
      ...
      toast({
        ...
        description: "Tài khoản đã được tạo thành công. Vui lòng đăng nhập để tiếp tục.",
        variant: "success",
        duration: 5000,
      });
  ...
  public async loginWithGoogle(googleData: { firebaseUid: string; email: string; name: string; avatar?: string }): Promise<void> {
    try {
      ...
      try {
        ...
        console.error('Failed to get user info after Google login:', userInfoError);
        // If we can't get user info, still mark as authenticated but with basic info
        this.state.user = {
          id: 0,
      ...
      this.state.isLoading = false;
      this.notifyListeners();
      console.log('Google login process completed, user authenticated:', this.state.isAuthenticated);
  ...
  public async registerWithGoogle(googleData: { email: string; name: string; avatar?: string; firebaseUid: string }): Promise<void> {
    try {
      console.log('Starting Google registration process for:', googleData.email, googleData.name);
      this.state.isLoading = true;
      this.notifyListeners();
