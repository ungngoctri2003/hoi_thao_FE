...
export function QRScanner({ onScanSuccess, onScanError, isScanning, onStartScan, onStopScan }: QRScannerProps) {
  ...
  const startCamera = async () => {
    try {
      ...
      if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        setError("Camera chỉ hoạt động trên HTTPS hoặc localhost. Vui lòng sử dụng HTTPS hoặc chạy trên localhost.");
      ...
      if (!QrScanner.hasCamera()) {
        setError("Không tìm thấy camera trên thiết bị này. Vui lòng kiểm tra thiết bị có camera không.");
        setHasPermission(false);
        return;
      }
      ...
      if (!videoRef.current) {
        setError("Video element không khả dụng. Vui lòng thử lại sau khi trang tải xong.");
        return;
      }
        ...
        <div className="space-y-4">
          ...
          <div className="relative w-full">
            <div className="bg-gray-100 rounded-lg overflow-hidden border-2 border-dashed border-gray-300" 
                 style={{ 
                   height: isScanning && hasPermission && isInitialized ? '70vh' : '400px',
                   minHeight: '400px',
                   maxHeight: '80vh'
                 }}>
              <div className="relative w-full h-full">
                <video
                  ...
                  muted
                  style={{ 
                    display: isScanning && hasPermission && isInitialized ? 'block' : 'none',
                    opacity: isScanning && hasPermission && isInitialized ? 1 : 0,
                    width: isScanning && hasPermission && isInitialized ? '100%' : '0px',
                    height: isScanning && hasPermission && isInitialized ? '100%' : '0px'
                  }}
                />
                {(!isScanning || !hasPermission || !isInitialized) && (
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Camera sẽ hiển thị toàn màn hình để quét QR code dễ dàng hơn
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    ...
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                      <div className="w-24 h-24 border-2 border-primary border-dashed rounded-lg bg-primary/10 flex items-center justify-center animate-pulse">
                        <div className="text-primary font-bold text-xs">SCANNING</div>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs">QR code có thể ở bất kỳ vị trí nào trong khung hình</p>
                </div>
              </div>
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs">Camera sẽ quét toàn bộ khung hình liên tục</p>
                </div>
              </div>
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs">QR code sẽ được phát hiện và quét tự động</p>
                </div>
              </div>
            </div>
