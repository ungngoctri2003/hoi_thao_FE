...
import { Calendar, Mail, Phone, MapPin, User, Building, Briefcase, Cake, Users, Plus, CheckCircle, XCircle, Clock, Trash2, Edit, QrCode } from "lucide-react";
import { Attendee, Conference, Registration, CheckinStatus } from "@/lib/api/attendees-api";
...
export function AttendeeDialog({ 
  ...
  const [pendingCheckinUpdates, setPendingCheckinUpdates] = useState<Map<number, string>>(new Map());
  ...
  const validateForm = () => {
    ...
    if (formData.FIREBASE_UID && !/^[a-zA-Z0-9_-]+$/.test(formData.FIREBASE_UID)) {
    ...
    try {
      ...
      if (pendingRegistrations.length > 0 && attendee?.ID) {
        ...
        for (const conferenceId of pendingRegistrations) {
          try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/attendees/${attendee.ID}/registrations`, {
              method: 'POST',
              headers: {
            ...
            if (!response.ok) {
              throw new Error(`Failed to register for conference ${conferenceId}`);
            }
            console.log(`Successfully registered for conference ${conferenceId}`);
          } catch (error) {
            console.error(`Error registering for conference ${conferenceId}:`, error);
       ...
       if (pendingCheckinUpdates.size > 0) {
         console.log('Processing pending checkin updates:', Array.from(pendingCheckinUpdates.entries()));
         for (const [registrationId, status] of pendingCheckinUpdates.entries()) {
           try {
             const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/registrations/${registrationId}`, {
               method: 'PATCH',
               headers: {
                 'Content-Type': 'application/json',
                 'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
             ...
             if (!response.ok) {
               throw new Error(`Failed to update registration ${registrationId} status to ${status}`);
             }
             console.log(`Successfully updated registration ${registrationId} status to ${status}`);
           } catch (error) {
             console.error(`Error updating registration ${registrationId}:`, error);
             throw new Error(`Lỗi khi cập nhật trạng thái đăng ký ${registrationId}`);
           }
         }
       }
       ...
       if (pendingDeletions.length > 0) {
         ...
         for (const registrationId of pendingDeletions) {
           try {
             ...
             if (!response.ok) {
               throw new Error(`Failed to delete registration ${registrationId}`);
             }
      ...
      if (mode === 'create' && pendingConferences.length > 0) {
       ...
       const totalChanges = pendingRegistrations.length + pendingCheckinUpdates.size + pendingDeletions.length;
       if (totalChanges > 0) {
         toast({
           title: "Thành công",
  ...
  const handleRemovePendingConference = (conferenceId: number) => {
    if (mode === 'create') {
      ...
      setPendingRegistrations(prev => prev.filter(id => id !== conferenceId));
    }
  };


  const handleUpdateCheckinStatus = (registrationId: number, status: string) => {
    setPendingCheckinUpdates(prev => new Map(prev.set(registrationId, status)));
  };

  const handleDeleteConference = (registrationId: number) => {
    // Use a more user-friendly confirmation approach
    const confirmed = window.confirm('Bạn có chắc chắn muốn xóa hội nghị này khỏi danh sách đăng ký?');
    if (confirmed) {
      setPendingDeletions(prev => [...prev, registrationId]);
      toast({
        title: "Thông báo",
        description: "Hội nghị đã được đánh dấu để xóa. Thay đổi sẽ được áp dụng khi lưu.",
        variant: "info",
      });
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    const config = statusConfig[status] || { label: status, color: "bg-gray-100 text-gray-600", icon: "❓" };
    return (
  ...
  const loadCheckinStatuses = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const statuses: CheckinStatus[] = data.data.map((reg: Registration) => ({
          registrationId: reg.ID,
          conferenceId: reg.CONFERENCE_ID,
          conferenceName: conferences.find(c => c.ID === reg.CONFERENCE_ID)?.NAME || 'Unknown Conference',
      ...
      <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] w-[95vw]">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2">
            ...
            <span>
              {mode === 'create' ? 'Thêm tham dự viên mới' : 
               mode === 'edit' ? 'Chỉnh sửa tham dự viên' : 
               'Chi tiết tham dự viên'}
            </span>
          </DialogTitle>
          <DialogDescription>
            {mode === 'create' ? 'Nhập thông tin tham dự viên mới' :
             mode === 'edit' ? 'Cập nhật thông tin tham dự viên' :
        ...
        <div className="space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {mode === 'create' ? 'Nhập thông tin cá nhân của tham dự viên mới' :
                 mode === 'edit' ? 'Cập nhật thông tin cá nhân' :
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {mode === 'create' ? 'Nhập thông tin công việc và tổ chức' :
                 mode === 'edit' ? 'Cập nhật thông tin công việc' :
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                {errors.FIREBASE_UID && <p className="text-sm text-red-500">{errors.FIREBASE_UID}</p>}
                <p className="text-xs text-muted-foreground">
                  Firebase UID được tự động tạo khi người dùng đăng nhập bằng Google
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {mode === 'create' ? 'Nhập thông tin bổ sung về tham dự viên' :
                 mode === 'edit' ? 'Cập nhật thông tin bổ sung' :
          ...
          {mode === 'view' && attendee && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Thông tin kỹ thuật và thời gian</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label>ID</Label>
                ...
                {attendee.FIREBASE_UID && (
                  <div className="space-y-2">
                    ...
                    <div className="p-2 bg-gray-100 rounded-md">
                      <span className="text-sm font-mono">{attendee.FIREBASE_UID}</span>
                    </div>
                  </div>
                )}
          ...
          <Card>
              <CardHeader>
                <CardTitle className="text-lg flex items-center justify-between">
                  <span>{isReadOnly ? 'Hội nghị và Check-in' : 'Quản lý hội nghị và Check-in'}</span>
                  {!isReadOnly && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setIsManagingConferences(!isManagingConferences)}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex space-x-2">
                      <Select
                        ...
                        onValueChange={(value) => setSelectedConferenceId(Number(value))}
                      >
                        ...
                        <SelectContent>
                          {conferences
                            .filter(conf => 
                              !checkinStatuses.some(status => status.conferenceId === conf.ID) &&
                ...
                <div className="space-y-4">
                   <div className="flex items-center justify-between">
                     <Label className="text-base font-semibold">
                       {mode === 'create' ? 'Hội nghị sẽ đăng ký' : 'Hội nghị đã đăng ký'} 
                       <span className="ml-2 text-sm font-normal text-muted-foreground">
                         ({mode === 'create' ? pendingConferences.length : checkinStatuses.length + pendingRegistrations.length - pendingDeletions.length})
                       </span>
                     </Label>
                  ...
                  <div className="space-y-4 max-h-96 overflow-y-auto border rounded-lg p-6 bg-gray-50/50">
                    ...
                    {mode === 'create' && pendingConferences.length > 0 ? (
                      pendingConferences.map((conferenceId) => {
                        ...
                        return (
                          <div key={conferenceId} className="p-4 border rounded-lg bg-blue-50/80 hover:bg-blue-100/80 transition-colors">
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-1">
                                  <Calendar className="h-4 w-4 text-blue-600" />
                                  <p className="font-semibold text-blue-900">{conference?.NAME || 'Unknown Conference'}</p>
                                </div>
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleRemovePendingConference(conferenceId)}
                     ...
                     ) : mode !== 'create' && (checkinStatuses.length > 0 || pendingRegistrations.length > 0) ? (
                       <>
                         {/* Show existing registrations */}
                         {checkinStatuses
                           .filter(status => !pendingDeletions.includes(status.registrationId))
                           .map((status) => (
                        <div key={status.registrationId} className="p-4 border rounded-lg bg-white hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-2">
                                <Calendar className="h-4 w-4 text-gray-600" />
                                <p className="font-semibold text-gray-900">{status.conferenceName}</p>
                              </div>
                              <div className="space-y-1 text-sm text-gray-600">
                                ...
                                <p className="flex items-center space-x-2">
                                  ...
                                  <span>Đăng ký: {formatDateTime(status.registrationDate)}</span>
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center space-x-2">
                              {getStatusBadge(pendingCheckinUpdates.get(status.registrationId) || status.status)}
                              {pendingCheckinUpdates.has(status.registrationId) && (
                          ...
                          {isManagingConferences && !isReadOnly && (
                            <div className="flex items-center space-x-3 pt-3 border-t border-gray-200">
                              <div className="flex-1">
                                <Select
                                  value={pendingCheckinUpdates.get(status.registrationId) || status.status}
                                  onValueChange={(value) => handleUpdateCheckinStatus(status.registrationId, value)}
                                  disabled={isLoading}
                                >
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem value="no-show">⏰ Không tham dự</SelectItem>
                                  </SelectContent>
                                </Select>
                              </div>
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleDeleteConference(status.registrationId)}
                                disabled={isLoading}
                                className="text-red-600 hover:text-red-700 hover:bg-red-50"
                              >
                              ...
                              <div className="space-y-1 text-xs text-gray-500">
                                {status.checkinTime && (
                                  <p className="flex items-center space-x-2">
                                    ...
                                    <span>Check-in: {formatDateTime(status.checkinTime)}</span>
                                  </p>
                                )}
                                {status.checkoutTime && (
                                  <p className="flex items-center space-x-2">
                                    <XCircle className="h-3 w-3 text-orange-600" />
                                    <span>Check-out: {formatDateTime(status.checkoutTime)}</span>
                                  </p>
                                )}
                              </div>
                            </div>
                         ...
                         {pendingRegistrations.map((conferenceId) => {
                           const conference = conferences.find(c => c.ID === conferenceId);
                           return (
                             <div key={conferenceId} className="p-4 border rounded-lg bg-yellow-50/80 hover:bg-yellow-100/80 transition-colors">
                               <div className="flex items-center justify-between">
                                 <div className="flex-1">
                                   <div className="flex items-center space-x-2 mb-1">
                                     <Calendar className="h-4 w-4 text-yellow-600" />
                                     <p className="font-semibold text-yellow-900">{conference?.NAME || 'Unknown Conference'}</p>
                                   </div>
                         ...
                         {pendingDeletions.map((registrationId) => {
                           const status = checkinStatuses.find(s => s.registrationId === registrationId);
                           return (
                             <div key={registrationId} className="p-4 border rounded-lg bg-red-50/80 hover:bg-red-100/80 transition-colors">
                               <div className="flex items-center justify-between">
                                 <div className="flex-1">
                                   <div className="flex items-center space-x-2 mb-1">
                                     <Calendar className="h-4 w-4 text-red-600" />
                                     <p className="font-semibold text-red-900">{status?.conferenceName || 'Unknown Conference'}</p>
                                   </div>
                                 ...
                                 <div className="flex items-center space-x-2">
                                   ...
                                   <Button
                                     size="sm"
                                     variant="outline"
                                     onClick={() => setPendingDeletions(prev => prev.filter(id => id !== registrationId))}
                                     className="text-red-600 hover:text-red-700 hover:bg-red-50"
                                   >
                                     <XCircle className="h-4 w-4" />
                                   </Button>
                      ...
                      <div className="p-8 text-center text-gray-500">
                        ...
                        <p className="text-lg font-medium mb-2">
                          {mode === 'create' ? 'Chưa chọn hội nghị nào' : 'Chưa đăng ký hội nghị nào'}
                        </p>
                        <p className="text-sm">
                          {mode === 'create' 
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-3 pt-6 border-t">
          <div className="flex-1">
            {mode === 'edit' && (pendingRegistrations.length > 0 || pendingCheckinUpdates.size > 0 || pendingDeletions.length > 0) && (
              <div className="flex items-center space-x-2 text-sm text-amber-700">
                <Clock className="h-4 w-4" />
                <span>
                  Có {pendingRegistrations.length + pendingCheckinUpdates.size + pendingDeletions.length} thay đổi chờ xử lý
                </span>
              </div>
            )}
          </div>
